...
const controlHistory = {
  redo: function (e) {
    ...
    if (ctr.type == "datachange") {
      ...
      Store.luckysheetfile[getSheetIndex(ctr.sheetIndex)].pivotTable =
        ctr.pivotTable;
      ...
      $("#luckysheet-sheets-item" + ctr.sheetIndex)
        .find(".luckysheet-sheets-item-name")
        .html(ctr.oldtxt);
      ...
      for (let i = 0; i < historyRules.length; i++) {
        ...
        ] = historyRules[i]["luckysheet_conditionformat_save"];
      ...
      Store.luckysheetfile[index]["luckysheet_alternateformat_save"] = $.extend(
        true,
        [],
        historyRules
      );
  ...
  undo: function () {
    ...
    if (ctr.type == "datachange") {
      ...
      //config
      Store.config = ctr.curconfig;
      Store.luckysheetfile[getSheetIndex(ctr.sheetIndex)].config =
        ctr.curconfig;
      ...
      for (let i = 0; i < currentRules.length; i++) {
        ...
        Store.luckysheetfile[getSheetIndex(sheetIndex)][
          "luckysheet_conditionformat_save"
        ] = currentRules[i]["luckysheet_conditionformat_save"];
