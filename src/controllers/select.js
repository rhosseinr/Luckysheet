...
function selectHightlightShow(isRestore = false) {
    ...
    for (let i = 0; i < Store.luckysheet_select_save.length; i++) {
      ...
      if (i == 0) {
        ...
        $("#luckysheet-cell-selected-boxs").append(
          '<div class="luckysheet-cell-selected" style="left: ' +
  ...
  ].luckysheet_select_save = Store.luckysheet_select_save;
  ...
  const luckysheet_select_save_previous = JSON.stringify(
    Store.luckysheet_select_save
  );

  if (
    (Store.luckysheet_select_save_previous == null) |
...
function selectTitlesShow(rangeArr, isRestore = false) {
  ...
  for (let i = 0; i < rowTitleRange.length; i++) {
    ...
    $("#luckysheet-rows-h-selected").append(
      '<div class="luckysheet-rows-h-selected" style="top: ' +
        row_pre +
        "px; height: " +
        (row - row_pre - 1) +
  ...
  for (let j = 0; j < columnTitleRange.length; j++) {
    ...
    $("#luckysheet-cols-h-selected").append(
      '<div class="luckysheet-cols-h-selected" style="left: ' +
        col_pre +
        "px; width: " +
        (col - col_pre - 1) +
        'px; display: block; background-color: rgba(76, 76, 76, 0.1);"></div>'
...
function collaborativeEditBox() {
  ...
  Store.cooperativeEdit.changeCollaborationSize.forEach((value) => {
    if (value.i == Store.currentSheetIndex) {
      ...
      // 改变列宽的位置在提示框范围内
      let flag_width =
        value.v.column[0] <= Store.luckysheet_cols_change_size_start[1] &&
        Store.luckysheet_cols_change_size_start[1] <= value.v.column[1];
      if (flag_width) {
      ...
      let flag_height =
        value.v.row[0] <= Store.luckysheet_rows_change_size_start[1] &&
        Store.luckysheet_rows_change_size_start[1] <= value.v.row[1];
      if (flag_height) {
      ...
      $("#luckysheet-multipleRange-show-" + value.id + ">.username").css({
...
function selectionCopyShow(range) {
  ...
  if (range.length > 0) {
    for (let s = 0; s < range.length; s++) {
      ...
      let copyDomHtml =
        '<div class="luckysheet-selection-copy" style="display: block; left: ' +
        col_pre +
        "px; width: " +
        (col - col_pre - 1) +
        "px; top: " +
...
//选区行列数显示
function luckysheet_count_show(
  left,
  top,
  width,
  height,
