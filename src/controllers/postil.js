...
const luckysheetPostil = {
  ...
  init: function () {
    ...
    $("#luckysheet-postil-showBoxs")
      ...
      .on("mousedown.showPs", ".luckysheet-postil-show", function (event) {
        if (
    ...
    $("#luckysheet-postil-showBoxs")
      .off("mouseup.showPs")
      .on("mouseup.showPs", ".luckysheet-postil-show", function (event) {
        if (event.which == "3") {
          event.stopPropagation();
        }
      });
    ...
    $("#luckysheet-postil-showBoxs")
      ...
      .on(
        "mousedown.resize",
        ".luckysheet-postil-show .luckysheet-postil-dialog-resize .luckysheet-postil-dialog-resize-item",
        function (event) {
          if (
            !checkProtectionAuthorityNormal(
              Store.currentSheetIndex,
          ...
          if (
            $(this)
              ...
              .hasClass("luckysheet-postil-show-active")
          ) {
            event.stopPropagation();
            return;
          }
    ...
    $("#luckysheet-postil-showBoxs")
      ...
      .on(
        "mousedown.move",
        ".luckysheet-postil-show .luckysheet-postil-dialog-move .luckysheet-postil-dialog-move-item",
        function (event) {
          if (
            !checkProtectionAuthorityNormal(
              Store.currentSheetIndex,
  ...
  overshow: function (event) {
    ...
    if (
      postil["isshow"] ||
      $("#luckysheet-postil-show_" + row_index + "_" + col_index).length > 0
    ) {
      return;
    }
    ...
    let ctx = $("#luckysheet-postil-overshow .arrowCanvas")
      .get(0)
      .getContext("2d");
  ...
  buildPs: function (r, c, postil) {
    ...
    if (isshow) {
      ...
      let html =
        ...
        size[1] +
        'px;z-index:100;pointer-events:none;"></canvas>' +
        '<div class="luckysheet-postil-show-main" style="width:' +
        width +
        "px;height:" +
        height +
        "px;color:#000;padding:5px;border:1px solid #000;background-color:rgb(255,255,225);position:absolute;left:" +
      ...
      let ctx = $("#luckysheet-postil-show_" + r + "_" + c + " .arrowCanvas")
        .get(0)
        .getContext("2d");
  ...
  editPs: function (r, c) {
    ...
    if ($("#luckysheet-postil-show_" + r + "_" + c).length > 0) {
      ...
      $("#luckysheet-postil-show_" + r + "_" + c).addClass(
        "luckysheet-postil-show-active"
      );
      $("#luckysheet-postil-show_" + r + "_" + c)
        .find(".luckysheet-postil-dialog-resize")
        .show();
    } else {
    ...
    $("#luckysheet-postil-show_" + r + "_" + c + " .formulaInputFocus").focus();
    luckysheetRangeLast(
      $("#luckysheet-postil-show_" + r + "_" + c + " .formulaInputFocus").get(0)
    );
      ...
      if (isAllShow) {
        ...
        for (let i = 0; i < allPs.length; i++) {
          ...
          if (!postil["isshow"]) {
            ...
            $(html).appendTo(
              $("#luckysheet-cell-main #luckysheet-postil-showBoxs")
            );

            let ctx = $(
              "#luckysheet-postil-show_" +
                rowIndex +
                "_" +
                colIndex +
                " .arrowCanvas"
  ...
  removeActivePs: function () {
    if (
      $("#luckysheet-postil-showBoxs .luckysheet-postil-show-active").length > 0
    ) {
      let id = $(
        "#luckysheet-postil-showBoxs .luckysheet-postil-show-active"
      ).attr("id");
